  ...
  if (payload.parentId && payload.parentChildRel) {
    ...
    if (twin) {
      ...
      for await (const rel of rels) {
        if (
          rel.sourceId === payload.parentId &&
          rel.relationshipName === payload.parentChildRel
        ) {
          relationship = rel;
          context.res.body = [
            ...context.res.body,
    ...
    if (!relationship) {
      try {
        ...
        const relResponse = await client.upsertRelationship(
          payload.parentId,
          $relationshipId,
          {
...
async function createTwin(payload: Payload, context: Context) {
  ...
  if (payload.id && payload.model) {
    ...
    const modelComponents = modelDef.model.contents?.filter(
      (content) => content["@type"] === "Component"
    );
    const modelProperties = modelDef.model.contents?.filter(
    ...
    try {
      const twinResponse = await client.upsertDigitalTwin(
        payload.id,
