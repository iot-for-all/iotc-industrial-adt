...
export const OpcuaViewer = React.memo(function OpcuaViewer({
  jsonContent,
  flatten,
  indentWidth,
  onSelect,
  ...
  const onMenuClick = React.useCallback(
    (e: MouseEvent, nodeKey: string, collapse: boolean) => {
      ...
      while (stack.length) {
        ...
        if (
          (currentNode as NSNode).children?.length &&
...
function OpcuaNodeList({
  ...
  const content = nodeRows.map((node) => {
    ...
    const tagNode: TagNode = (node as TagNode).type
      ? (node as TagNode)
      : undefined;
    ...
    const type = tagNode?.type ? (
      tagNode.type === "complex" ? (
        ...
        <span>
          (<span style={styles?.type}>{tagNode.type}</span>)
        </span>
      )
    ) : (
      ""
    ...
    if (flatten && type) {
      return (
        <div
          key={fullName}
          title={fullName}
          className="row vertical-group clickable viewer-card margin-bottom-xsmall font-small"
          onClick={select}
        >
          <div className="horizontal-group justify-ends">
            <div
              className={`font-small selectable ${
                selectedItemKey === tagNode.key ? "selected" : ""
              }`}
      ...
      return (
        <div
          ...
          className={`row font-small margin-bottom-xsmall ${
            selectedItemKey === node.key ? "selected" : ""
          }`}
          style={style}
        >
          {!tagNode && <MenuIcon />}
          <div
            ...
            style={
              tagNode?.type
                ? styles?.leafName || styles?.nodeName
                : styles?.nodeName
            }
          >
            {(node as TagNode).name ?? node.id}
          ...
          {!!(node as TagNode).name && (
            <div className="margin-start-xsmall">{node.id}</div>
          )}
        </div>
      );
    }
  ...
  return <div className="viewer">{content}</div>;
}
