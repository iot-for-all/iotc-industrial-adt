...
export const DtModelViewer = React.memo(function DtModelViewer(
  props: DtModelViewerProps
) {
  const {
    models,
    indentWidth,
      ...
      searchFilter
        ? inputRows.filter((row) => {
            let keep =
              ...
              row.namespace?.includes(searchFilter) ||
              row.name?.includes(searchFilter) ||
              row.type?.includes(searchFilter) ||
            ...
            if (!keep) {
              if (typeof row.displayName === "string") {
                ...
                keep = (row.displayName["en"] as string)?.includes(
                  searchFilter
                );
              }
            }
...
function useProcessJson(models: (Interface | Model)[]): ProcessedInput {
  return React.useMemo(() => {
    ...
    for (const rawNode of rootArray) {
      ...
      if (rawNode.contents) {
        rawNode.contents.forEach((childNode) =>
          processObject(childNode, nodeMap, idToKeyMap, node, collapsed)
        );
      }
    }
...
function processObject(
  ...
  const namespace = [
    ...
    parentNode.name ?? parentNode.displayName ?? parentNode.id,
  ];
  ...
  switch (type?.toLowerCase()) {
    case "property":
      ...
      switch (schema.toLowerCase()) {
        ...
        case "enum":
        case "map":
          node.complexValues = getComplexValues(
            rawNode.schema,
            schema.toLowerCase()
          );
          break;
...
function getSchemaName(
  schema: string | PropertySchema | ComplexObjectSchema
): string {
...
function getObjectFields(schema: any, depth: number): ComplexProperties[] {
  return schema.fields.map((field) => {
    ...
    const properties =
      fieldSchema.toLowerCase() === "object"
        ? getObjectFields(field.schema, depth + 1)
        : [];
    const complexValues =
      fieldSchema.toLowerCase() === "object"
        ? []
        : getComplexValues(field.schema, fieldSchema);
    return {
      name: field.name,
      schema: fieldSchema,
      properties,
...
function useGetRows(processedInput: ProcessedInput): Node[] {
  return React.useMemo(() => {
    ...
    while (stack.length) {
      ...
      node.children.map((childNode) => {
        ...
        if (type === "component" || type === "relationship") {
          ...
          const compInterfaceKey =
            idToKeyMap[
              type === "component" ? childNode.schema : childNode.target
            ];
          ...
          childNode.displayName =
            typeof childNode.displayName === "string"
              ? childNode.displayName
    ...
    const topLevelInterfaces = rootNodes.filter(
      (nodeId) => !nestedInterfaces.includes(nodeId)
    );
    const updateCollapsed =
      rootNodes.length > 1 && topLevelInterfaces.length === 1;
...
function ModelsList({
  ...
  const content = nodeRows.map((node) => {
    const fullName = [
      ...node.namespace,
      node.name ?? node.displayName ?? node.id,
    ].join(".");
    ...
    const select =
      node.type.toLowerCase() === "property"
        ? () => onSelect(selectedModelKey === node.key ? undefined : node)
        : undefined;
    if (!node.hide || filtered) {
      const style: React.CSSProperties = {
        paddingInlineStart: `${indentPixels * node.depth}px`,
      };
      return (
        <div
          key={fullName}
          title={fullName}
          className={`row font-small margin-bottom-xsmall ${
            selectedModelKey === node.key ? "selected" : "unselected"
          }`}
          style={style}
        >
          ...
          <div
            ...
            onClick={select}
          >
            {node.type.toLowerCase() === "interface" && (
              <div className="interface">
                {node.type}:{" "}
                <span style={styles?.interfaceId}>{node.name ?? node.id}</span>
              </div>
            )}
            {node.type.toLowerCase() === "property" && (
              <div>
                ...
                {["object", "map", "enum"].includes(
                  node.schema.toLowerCase()
                ) ? (
                  <Details node={node} styles={styles} />
                ) : (
                  <span>
                    (
                    <span style={styles?.propertySchema}>
                      {node.schema.toLowerCase() === "array"
                        ? `${node.complexValues[0]} `
                        : ""}
                      {node.schema}
                    </span>
                    )
            ...
            {node.type.toLowerCase() === "component" && (
              <div>
                <div>
                  {node.type}:{" "}
                  <span style={styles?.componentName}>{node.name}</span>
                </div>
                <div style={styles?.interfaceId}>{node.schema}</div>
              </div>
            )}
            {node.type.toLowerCase() === "relationship" && (
              <div>
                <div>
                  {node.type}:{" "}
                  <span style={styles?.relationshipName}>
                    {node.name}{" "}
...
export const Details = React.memo(function Details({
      ...
      content = (
        <>
          {node.object.map((detail, idx) => {
            const style: React.CSSProperties = {
              paddingInlineStart: `${defaultIndent * detail.depth}px`,
            };
            const schema =
              detail.schema.toLowerCase() === "array"
                ? `${detail.complexValues[0]} Array`
                : detail.schema;

            return (
              <div key={`${generateId()}-${idx}`}>
                <div style={style}>
                  {detail.name} <span>({schema})</span>
                </div>
                {schema.toLowerCase() === "object" &&
                  getObjectSchemaJSX(detail)}
                {["enum", "map"].includes(schema.toLowerCase()) &&
                  getComplexSchemaJSX(detail.complexValues, schema)}
              </div>
            );
          })}
        </>
  ...
  return (
    <TooltipHost
      ...
      id={tooltipId}
    >
      <span className="details" key={node.key}>
        (<span style={styles?.propertySchema}>{node.schema}</span>)
      </span>
    </TooltipHost>
  );
});
