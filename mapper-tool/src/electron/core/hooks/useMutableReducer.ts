...
export type MutableReducer<S, A extends unknown[]> = (
  state: S,
  ...args: A
) => S | void;

/** Returns the reducer function type without the state argument */
export type ExcludeState<S, R> = R extends MutableReducer<S, infer A>
  ? (...args: A) => void
  : never;

/** A collection of reducers */
export type MutableReducers<S> = {
  [type: string]: MutableReducer<S, unknown[]>;
};
...
export type MutableReducerResult<S, R extends MutableReducers<S>> = [
  S,
  ImmutableDispatchers<S, R>
];
 ...
 * </form>
 */
export function useMutableReducer<S, R extends MutableReducers<S>>(
  reducers: R,
  initialState: S | ((dispatch?: ImmutableDispatchers<S, R>) => S)
) {
  const [state, setState] = React.useState<MutableReducerResult<S, R>>(() => {
    function dispatchAction(type: string, ...args: unknown[]) {
      const reducer = reducers[type];
      setState((prevState) => {
        const [prev, dispatch] = prevState;
        const next = produce(
          prev,
          (draft) => reducer(draft as S, ...args) as Draft<S>
        ) as S;
        return Object.is(prev, next) ? prevState : [next, dispatch];
      });
    }

    const dispatch: ImmutableDispatchers<S, R> = {};
    for (const type in reducers) {
      dispatch[type] = dispatchAction.bind(null, type);
    }

    const state =
      initialState instanceof Function ? initialState(dispatch) : initialState;
