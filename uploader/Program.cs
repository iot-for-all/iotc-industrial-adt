...
namespace ModelUploader
{
    public class Program
    {
        ...
        private static string modelPath = "ontology";
        ...
        static void Main(string[] args)
        {
            ...
            var deletefirstenv = Environment.GetEnvironmentVariable("DELETE_MODELS_FIRST");
            if (deletefirstenv != null && deletefirstenv.Equals("true"))
            {
                deleteFirst = true;
            }
            else
            ...
            {
                Log.Out("Cannot read configuration file. Skipping...");
                // try with env variable in case
                clientId = Environment.GetEnvironmentVariable("CLIENT_ID");
                clientSecret = Environment.GetEnvironmentVariable("CLIENT_SECRET");
                tenantId = Environment.GetEnvironmentVariable("TENANT_ID");
                adtInstanceUrl = Environment.GetEnvironmentVariable("ADT_INSTANCE");
            }

            if (String.IsNullOrEmpty(adtInstanceUrl))
            {
                Log.Error($"Missing Azure Digital Twin instance host.");
                Log.Alert($"Set 'ADT_INSTANCE' environment variable or specify the value as 'instanceUrl' in the serviceConfig configuration file");
                Environment.Exit(0);
            }
            ...
            try
            {
                ...
                if (tenantId != null && tenantId.Length > 0)
                {
                    ...
                    {
                        client = new DigitalTwinsClient(new Uri(adtInstanceUrl), new ClientSecretCredential(tenantId, clientId, clientSecret));
                    }
                    else
                    {
                ...
                catch (Exception e)
                {
                    ...
                    Log.Alert($"Make sure you followed authentication instructions on repo.");
                    Environment.Exit(0);
                }
            }
            catch (Exception e)
