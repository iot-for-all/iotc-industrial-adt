...
export const DtTwinsViewer = React.memo(function DtTwinsViewer(props: TwinsViewerProps) {
    ...
    const { twins: jsonContent, onSelect, selectedModelId, selectedTwinKey, styles, customTwin, searchFilter } = props;
                ...
                <div key={node.key} className={`${baseClass} ${modelClass} ${separatorClass} ${selectedClass}`} >
                    ...
                    <div
                        ...
                        {!(node.isModel || node.isSeparator) && <div>
                            <div>{node.isNew ? 'Future ' : ''}Twin: <span style={styles?.twinId}>{node.id}</span></div>
