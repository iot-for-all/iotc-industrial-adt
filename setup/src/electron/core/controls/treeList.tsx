...
import {
  ImmutableDispatchers,
  useMutableReducer,
} from "../hooks/useMutableReducer";
...
type TreeListColumn<T = any> = CustomColumnOptions<T> &
  Pick<IColumn, "key" | "name" | "fieldName" | "isResizable">;
...
const reducers = {
  initialize(
    ...
    dispatch: ImmutableDispatchers<TreeViewState, any>,
    options: InfiniteScrollTreeOptions2<any>
  ): TreeViewState {
    const {
      columns: gridColumns,
    ...
    const parentMenuState: ParentMenuState =
  ...
  toggleParentMenuState(state: TreeViewState, parentRowKey: string) {
    ...
    if (parentRowKey) {
      ...
      const newState = !currentMenuState
        ? MenuState.Collapsed
        : MenuState.Expanded;
      state.parentMenuState = {
        ...state.parentMenuState,
        [parentRowKey]: newState,
      };
      state.collapsedParents =
        newState === MenuState.Expanded
...
export function useTreeList<T>(options: InfiniteScrollTreeOptions2<T>) {
  const store = useMutableReducer<TreeViewState<T>, typeof reducers>(
    reducers,
    (dispatch) => reducers.initialize(null, dispatch, options)
  );
...
export function TreeList<T>(props: {
  ...
  const { options, columns, selection, key, collapseTree, parentMenuState } =
    state;
  const {
    ariaLabel,
    dataTestHook,
  ...
  const onMenuRender = React.useCallback(
    (item?: any) => {
  ...
  React.useEffect(() => {
    ...
    if (!state.collapseInitialized.hasMenuColumn) {
        ...
        return (
          <div className={"indented-container"}>
            ...
            <div style={style} className={"inline-text-overflow"}>
              {col1.onRender(item)}
              {!!item?.count && (
                <span className={"descendant-count"}> ({item.count})</span>
              )}
            </div>
          </div>
        );
      ...
      iCol.minWidth =
        (col1.minWidth || DefaultColumnWidth) + maxDepth * indentSize;
    ...
    maxDepth,
    onMenuRender,
    state.collapseInitialized.hasMenuColumn,
  ]);
  ...
  React.useEffect(() => {
    ...
    if (!state.collapseInitialized.hasCollapsedState && inputItems.length) {
      ...
      for (const item of inputItems) {
        if (item.childrenIds?.length) {
          parentStates[item.id] = startCollapsed
            ? MenuState.Collapsed
            : MenuState.Expanded;
        }
      }
    ...
    inputItems,
    startCollapsed,
    state.collapseInitialized.hasCollapsedState,
  ]);

  const collapsedItems = React.useMemo(
    () => getVisibleRows(inputItems, parentMenuState, collapseTree),
    [collapseTree, inputItems, parentMenuState]
  );
        ...
        ? {
            onClick: (e) => {
              ...
              if (!!rowProps.item.childrenIds.length) {
                ...
                !!state.selection.isKeySelected(rowProps.item.parentId)
              ) {
          ...
          onRenderCheck={(checkboxProps: IDetailsRowCheckProps) =>
            !hideCheckbox && (
              <DetailsRowCheck
                ...
                {...selectionProps}
                data-test-hook={
                  getRowCheckboxTestHook?.(rowProps.item, rowProps.itemIndex) ??
                  getRowKey?.(rowProps.item, rowProps.itemIndex) ??
                  `checkbox-${rowProps.itemIndex}`
                }
              />
            )
          }
  ...
  const onActiveItemChanged = React.useCallback(
    (item?: any, _?: number, ev?: React.FocusEvent<HTMLElement>) => {
      item &&
        !item.disabled &&
        !ev?.target.onclick &&
        onItemSelect?.({
  ...
  const onRenderDetailsHeader = React.useCallback(
    (
      ...
      defaultRender?: (props: IDetailsHeaderProps) => React.ReactNode
    ) => {
  ...
  return (
    typeof window !== "undefined" && (
      <div data-test-hook={dataTestHook}>
        <ShimmeredDetailsList
          ...
          onRenderDetailsHeader={
            props.onRenderDetailsHeader || onRenderDetailsHeader
          }
          selection={selection}
        ...
        {enableEmptyPlaceholder && !collapseTree && (
          <div
            className={"inline-text-overflow no-rows-found"}
            data-test-hook="no-rows-found"
          >
...
function selectChildren(state: TreeViewState) {
  ...
  if (selectedItems.length > 1) {
    ...
    const highestParents = parents.filter(
      (parent) => !parentIds.includes(parent.parentId)
    );
    const items = state.selection.getItems();

    // get the selected indices as a map for lookup
    const selectedIndexes = state.selection
      .getSelectedIndices()
      .reduce((map, idx) => {
        map[idx] = true;
        return map;
    ...
    for (const item of highestParents) {
      ...
      if (shouldToggle) {
        state.selection.toggleRangeSelected(
          parentIndex,
...
function getVisibleRows(
  items: any[],
  parentStateMenu: ParentMenuState,
  collapseTree: boolean
) {
  if (!parentStateMenu) {
    return items;
...
const CollapsibleMenu = React.memo(function CollapsibleMenu({
  row,
  style,
  options,
  dispatch,
  ...
  return (
    <div className={"collapse-menu-container"} style={style}>
      {!!row.childrenIds?.length && (
        <div className={"collapse-menu"} data-selection-disabled>
          <TooltipIconButton
...
function defaultRenderer<T>(
  ...
  return text == null ? null : (
    <span title={text} data-test-hook={dataTestHook?.(item)}>
      {text}
    </span>
  );
}

function FetchTriggerRow({
  callback,
  children,
}: {
  callback: () => void;
